            ...
            for i, item in ipairs(completionitems) do
                ...
                if string.startsWith(item.filterText or item.label, stem) then
                    ...
                    elseif item.additionalTextEdits then
						...
						-- ORIGINAL:
						if bufpane.Buf:FileType() ~= "java" then

							-- get the 'compact_hints' flag or use false by default
							local showCompact = bufpane.Buf.Settings["mlsp.compact_hints"]
							if showCompact == nil then showCompact = false end

							if showCompact then
                        		table.insert(labels, item.label)
                        	else
                        		-- ONLY SHOWS SUGGESTIONS NAMES
	   	                        local cleanLabel = item.filterText or item.insertText or item.label
	   	                        table.insert(labels, cleanLabel)
   	                        end
                        end
                        
                        -- CUSTOM INFORMATION FOR JDTLS:
                        -- SIMPLE
                        -- variable: name [v]
                        -- function: name [f]
                        -- DETAILED
                        ...
                        local name = item.filterText or item.insertText or item.label
                        local cleanLabel = name

                        -- Check if the detailed mode is enabled in user settings (defaults to true if nil)
                        local formatFlag = bufpane.Buf.Settings["mlsp.java_hints_format"]
						-- true by default
                        local showDetailed = true

						-- false only if specified to be simple
                        if formatFlag == "simple" then
                        	showDetailed = false
                        end
                        ...
                        if bufpane.Buf:FileType() == "java" then
                            if showDetailed then
                                -- --- DETAILED MODE ---
                                -- 1. Determine the visibility by extracting detail or label information
                                local vis = "i" -- default package
                                local detail = (item.labelDetails and item.labelDetails.detail) or item.detail or ""
                                local fullText = item.label .. " " .. detail

                                if fullText:find("public") or fullText:find("▲") then vis = "p"
                                elseif fullText:find("private") or fullText:find("▼") then vis = "r"
                                elseif fullText:find("protected") or fullText:find("⬧") then vis = "c"
                                end

                                -- 2. Determine the modifiers (static, final)
                                local isStatic = fullText:find("static") ~= nil
                                local isFinal = fullText:find("final") ~= nil

                                -- 3. Identify the element type (LSP CompletionItemKind)
                                -- 2 = Method, 3 = Function, 4 = Constructor
                                if item.kind == 2 or item.kind == 3 or item.kind == 4 then
                                    -- Extract the parameters inside the brackets
                                    local params = item.label:match("%((.-)%)") or ""
                                    params = params:gsub("([%w_]+%. )", "")
                                    
                                    -- Extract return type, handling nested Generics safely
                                    local retType = item.label:match(":%s*(.+)$") or detail:match("^%s*(.+)$") or "void"
                                    retType = retType:gsub("[%w_]+%.", "")
                                    retType = retType:gsub("^%s+", ""):gsub("%s+$", "")

                                    cleanLabel = string.format("%s: [%s: (%s) -> %s]", name, vis, params, retType)
                                else
                                    -- Treat the element as a variable/field
                                    local varType = detail:match("^%s*(.+)$") or item.label:match(":%s*(.+)$") or "Object"
                                    varType = varType:gsub("[%w_]+%.", "")
                                    varType = varType:gsub("^%s+", ""):gsub("%s+$", "")

                                    -- Apply the modifiers prefix
                                    local mod = "v"
                                    if isStatic and isFinal then mod = "vsf"
                                    elseif isFinal then mod = "vf"
                                    elseif isStatic then mod = "vs"
                                    end

                                    cleanLabel = string.format("%s [%s%s: %s]", name, mod, vis, varType)
                                end
                            else
                                -- --- SIMPLE MODE ---
                                -- Identify element type (2 = Method, 3 = Function, 4 = Constructor)
                                if item.kind == 2 or item.kind == 3 or item.kind == 4 then
                                    -- Function format: name [f]
                                    cleanLabel = string.format("%s [f]", name)
                                else
                                    -- Variable format: name [v]
                                    cleanLabel = string.format("%s [v]", name)
                                end
                            end
                        end
